...
async function sendPasswordResetOTPEmail(userEmail, userName, otpCode, expiryMinutes) {
  const subject = 'Reset your Mandal-Book password';
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #111827;">
      ...
      <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
        <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">Hello <strong>${userName}</strong>,</p>
        <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">
          You requested to reset the password for your Mandal-Book account. Please use the following one-time code to continue:
        </p>
        <div style="background: linear-gradient(135deg, #059669 0%, #10b981 100%); padding: 20px; text-align: center; border-radius: 8px; margin: 30px 0;">
          <p style="font-size: 36px; font-weight: bold; letter-spacing: 8px; color: white; margin: 0; font-family: 'Courier New', monospace;">
            ${otpCode}
          </p>
        </div>
...
async function sendContributionReminderEmail(userEmail, userName, currentMonth, dayOfMonth) {
  ...
  // Get payment settings from database only (no env fallback)
  let qrCodeUrl = null; // static QR uploaded by admin (fallback)
  let upiId = null;
  let monthlyAmount = null;
  let upiUrl = null;
  let upiQrUrl = null; // dynamic QR for this month's UPI deep link
  ...
  try {
    ...
    if (qrCodeSetting?.value) qrCodeUrl = qrCodeSetting.value;
    if (upiIdSetting?.value) upiId = upiIdSetting.value;
    if (monthlyAmountSetting?.value !== undefined && monthlyAmountSetting?.value !== null) {
      const parsed = Number(monthlyAmountSetting.value);
      if (!Number.isNaN(parsed)) {
        monthlyAmount = parsed;
      }
    }

    // Build dynamic UPI link & QR only if we have both UPI ID and monthly amount
    if (upiId && monthlyAmount) {
      // Derive human-friendly month label from currentMonth (YYYY-MM)
      try {
        const dateForMonth = new Date(`${currentMonth}-01T00:00:00`);
        if (!isNaN(dateForMonth)) {
          const monthName = dateForMonth.toLocaleString('default', { month: 'short' });
          const year = dateForMonth.getFullYear();
          monthLabel = `${monthName}-${year}`;
        }
      } catch (_) {
        // fallback: keep currentMonth as label
      }

      const upiName = 'Mandal Group';
      const note = `Mandal Contribution ${monthLabel}`;
      upiUrl = `upi://pay?pa=${encodeURIComponent(
        upiId
      )}&pn=${encodeURIComponent(upiName)}&am=${monthlyAmount}&cu=INR&tn=${encodeURIComponent(
        note
      )}`;

      // Use a public QR generator service for email embedding
      upiQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(
        upiUrl
      )}`;
    }
  } catch (error) {
    console.error('Error fetching payment settings for email:', error);
    // No fallback - only use database values
  }
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #111827;">
      ...
      <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
        <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">Hello <strong>${userName}</strong>,</p>
        <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">
          This is a friendly reminder that your contribution for <strong>${monthLabel}</strong> is pending.
        </p>
        ${
          monthlyAmount
            ? `<p style="font-size: 16px; color: #111827; margin-bottom: 16px;">
                 Expected monthly contribution amount: <strong>₹${monthlyAmount.toLocaleString(
                   'en-IN'
                 )}</strong>
               </p>`
            : ''
        ...
        ${upiQrUrl || qrCodeUrl || upiId ? `
        <!-- Payment Details Section -->
        <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 25px 0;">
          <h2 style="font-size: 18px; font-weight: 600; color: #111827; margin: 0 0 15px 0; text-align: center;">Payment Details</h2>
          <div style="text-align: center;">
            ${upiQrUrl || qrCodeUrl ? `
            <div style="margin-bottom: 20px;">
              <p style="font-size: 14px; color: #6b7280; margin-bottom: 10px;">Scan QR Code to Pay</p>
              <div style="display: inline-block; background: white; padding: 10px; border-radius: 8px; border: 2px solid #e5e7eb;">
                <img src="${upiQrUrl || qrCodeUrl}" alt="Payment QR Code" style="width: 180px; height: 180px; display: block;" />
              </div>
            </div>
            ` : ''}
            ${upiId ? `
            <div style="background: linear-gradient(135deg, #dbeafe 0%, #e0e7ff 100%); border: 1px solid #3b82f6; border-radius: 8px; padding: 15px; margin-top: 15px;">
              <p style="font-size: 12px; color: #6b7280; margin: 0 0 8px 0; text-transform: uppercase; letter-spacing: 0.5px;">UPI ID</p>
              <p style="font-size: 18px; font-weight: 600; color: #1e40af; margin: 0; word-break: break-all;">${upiId}</p>
            </div>
            ` : ''}
          </div>
        </div>
