...
/**
 * Decrypt dates in request body (server-side)
 * This should be called in API endpoints before processing request data
 */
function decryptRequestDates(req) {
  if (!req.body || typeof req.body !== 'object') {
    return;
  }

  const { decryptDatesInObject } = require('./dateEncryption');
  req.body = decryptDatesInObject(req.body);
}
